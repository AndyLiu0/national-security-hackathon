...
SENSOR_TIERS = {
    # ── ATTRITABLE COTS TIERS (sub-$10K target) ──
    'cots_minimal': {
        'description': 'COTS minimal - MEMS infrasound + thermal module',
        'mass_kg': 0.8,
        'always_on_W': 5,
        'peak_W': 12,
        'cost_usd': 800,
        ...
        'swarm_fusion_boost': 0.25,  # probability boost per overlapping node
        'components': ['MEMS infrasound x3 ($150)', 'FLIR Lepton 3.5 ($250)',
                       'RPi Zero 2W ($15)', 'GPS + IMU ($50)'],
    },
    'cots_standard': {
        'description': 'COTS standard - infrasound array + SWIR + edge AI',
        'mass_kg': 1.5,
        'always_on_W': 10,
        'peak_W': 25,
        'cost_usd': 2500,
        'detection_range_km': 50,
        'detection_probability': 0.52,
        'swarm_fusion_boost': 0.22,
        'components': ['MEMS infrasound x6 ($300)', 'SWIR InGaAs module ($800)',
                       'RPi CM4 + Coral TPU ($120)', 'EO cam 12MP ($80)',
                       'GPS/INS + LoRa radio ($200)'],
    },
    'cots_enhanced': {
        'description': 'COTS enhanced - full COTS suite + onboard ML',
        'mass_kg': 2.5,
        'always_on_W': 18,
        'peak_W': 40,
        'cost_usd': 5000,
        ...
        'swarm_fusion_boost': 0.18,
        'components': ['MEMS infrasound x8 precision ($500)', 'SWIR InGaAs array ($1500)',
                       'Jetson Orin Nano ($250)', 'EO cam stabilized ($300)',
                       'Multi-band radio + GPS/INS ($400)'],
    },
    # ── MILITARY-GRADE TIERS (original, for reference) ──
    'mil_standard': {
        'description': 'Mil-spec - precision infrasound + SWIR array + EO',
        'mass_kg': 6.0,
        'always_on_W': 35,
        'peak_W': 75,
        'cost_usd': 85000,
        ...
        'swarm_fusion_boost': 0.10,
        'components': ['Precision infrasound array', 'SWIR wide-angle cooled',
                       'EO sentinel camera', 'M1-class edge compute'],
    },
    'mil_premium': {
        'description': 'Mil-spec premium - full multi-spectral + AI',
        'mass_kg': 10.0,
        'always_on_W': 60,
        'peak_W': 150,
        'cost_usd': 220000,
        ...
        'swarm_fusion_boost': 0.05,
        'components': ['Redundant infrasound', 'Cooled SWIR array',
                       'Stabilized EO gimbal', 'MWIR', 'Edge TPU'],
    },
}
...
class CommsConfig:
    ...
    satellite_link: bool = False           # COTS default: mesh-only, no satcom
    node_to_node_range_km: float = 200     # LOS from 60k ft
    bandwidth_mbps: float = 1.0            # LoRa + lightweight data link
    comms_mass_kg: float = 0.3             # COTS radio module
    comms_power_W: float = 3.0             # low-power mesh radio
    comms_cost_usd: float = 400            # LoRa + antenna
    relay_level: str = "mesh_relay"


# Pre-built comms configs
COMMS_COTS = CommsConfig()  # default COTS
COMMS_MILSPEC = CommsConfig(
    satellite_link=True, node_to_node_range_km=300, bandwidth_mbps=5.0,
    comms_mass_kg=1.5, comms_power_W=12.0, comms_cost_usd=15000,
    relay_level="mesh_with_satcom_backup"
)
...
def compute_unit_cost(structural_mass_kg: float, power_system_mass_kg: float,
                      ...
                      payload_cost_usd: float, wingspan_m: float,
                      production_run: int = 200) -> dict:
    """
    Estimate per-unit manufacturing cost. production_run=200 for swarm scale.
    """
    # Airframe cost depends on production scale and method
    # At scale (100+ units): automated layup ~$200/kg
    # Small batch (10-50): hand layup ~$450/kg
    if production_run >= 100:
        cf_cost_per_kg = 200
    else:
        cf_cost_per_kg = 350
    ...
    tooling_cost = 30000 + wingspan_m * 1000
    tooling_per_unit = tooling_cost / production_run

    # Solar cells: cost scales with area
    # GaAs: ~$100-150/m² at volume; Si flexible film: ~$30-60/m²
    # For COTS builds, use flexible Si; for mil-spec, GaAs
    is_cots = payload_cost_usd < 10000
    solar_cost_per_m2 = 40 if is_cots else 120
    solar_cost = solar_area_m2 * solar_cost_per_m2

    # Battery cost
    battery_cost_per_kWh = 150  # Li-S at volume
    battery_cost = (battery_energy_Wh / 1000) * battery_cost_per_kWh

    # Power electronics: COTS MPPT + BMS vs mil-spec
    power_electronics_cost = 200 if is_cots else 2000

    # Assembly, integration, test
    ait_cost = 500 if is_cots else airframe_cost * 0.25

    # Propulsion (COTS brushless motors + props)
    propulsion_cost = 300 if is_cots else 2000

    # Recovery hardware (landing skid, GPS return logic)
    recovery_cost = 200 if is_cots else 500

    per_unit_cost = (airframe_cost + tooling_per_unit + solar_cost + battery_cost +
                     power_electronics_cost + payload_cost_usd + ait_cost +
                     propulsion_cost + recovery_cost)
...
def compute_fleet(detection_range_km: float, coverage_target_km2: float,
                  per_unit_cost: float, single_node_pd: float = 0.5,
                  swarm_fusion_boost: float = 0.20,
                  overlap_factor: float = 1.5,
                  spares_fraction: float = 0.20,
                  endurance_days: float = 7.0) -> dict:
    """
    Compute fleet sizing for area coverage with swarm fusion detection.
    Higher overlap = more nodes see same target = better fused detection.
    """
    ...
    # Swarm fusion: average number of overlapping nodes at any point
    avg_overlap = overlap_factor  # by definition
    # Fused detection probability: 1 - (1-p_single)^n_overlap (independent sensors)
    # Plus a fusion intelligence boost for correlated multi-sensor processing
    n_overlap = max(1, round(avg_overlap))
    p_independent = 1.0 - (1.0 - single_node_pd) ** n_overlap
    # Fusion boost: multi-node correlation improves beyond independent assumption
    p_fused = min(0.99, p_independent + swarm_fusion_boost * (n_overlap - 1))
    swarm_detection_probability = p_fused
    ...
    return {
        ...
        'single_node_pd': single_node_pd,
        'avg_overlap_nodes': n_overlap,
        'swarm_detection_pd': swarm_detection_probability,
        'swarm_fusion_boost': swarm_fusion_boost,
    }
...
def evaluate_design(wingspan_m: float, chord_m: float, sensor_tier: str,
    ...
    # Fleet
    # Get swarm fusion boost from sensor tier
    sensor_data = SENSOR_TIERS[sensor_tier]
    sfb = sensor_data.get('swarm_fusion_boost', 0.10)

    fleet = compute_fleet(payload['detection_range_km'], coverage_target_km2,
                         cost['per_unit_cost_usd'],
                         single_node_pd=payload['detection_probability'],
                         swarm_fusion_boost=sfb)
...
def optimize_design(altitude_ft: float = 78000,
    ...
    # Design space
    wingspans = [i * 1.0 for i in range(8, 36)]       # 8m to 35m in 1m steps
    chords = [i * 0.05 for i in range(6, 30)]          # 0.3m to 1.45m in 0.05m steps
    altitudes = [i * 1000 for i in range(int(altitude_range[0]/1000), int(altitude_range[1]/1000) + 1)]
    tiers = ['cots_minimal', 'cots_standard', 'cots_enhanced', 'mil_standard', 'mil_premium']
...
def sensitivity_analysis(base_wingspan: float, base_chord: float,
    ...
    ws_data = []
    for ws in [base_wingspan * f for f in [0.8, 0.9, 1.0, 1.1, 1.2]]:
        try:
            d = evaluate_design(ws, base_chord, sensor_tier, altitude_ft)
            if d is not None:
                ws_data.append({'wingspan_m': ws, 'mass': d['mass_breakdown']['total_mass_kg'],
                    'cost': d['cost']['per_unit_cost_usd'], 'feasible': d['feasibility']['all_feasible']})
        except Exception:
            pass
    sensitivities['wingspan'] = ws_data

    ch_data = []
    for ch in [base_chord * f for f in [0.8, 0.9, 1.0, 1.1, 1.2]]:
        try:
            d = evaluate_design(base_wingspan, ch, sensor_tier, altitude_ft)
            if d is not None:
                ch_data.append({'chord_m': ch, 'mass': d['mass_breakdown']['total_mass_kg'],
                    'cost': d['cost']['per_unit_cost_usd'], 'feasible': d['feasibility']['all_feasible']})
        except Exception:
            pass
    ...
    alt_data = []
    for alt in range(55000, 82000, 3000):
        try:
            d = evaluate_design(base_wingspan, base_chord, sensor_tier, alt)
            if d is not None:
                alt_data.append({'alt_ft': alt, 'mass': d['mass_breakdown']['total_mass_kg'],
                    'power_W': d['aerodynamics']['power_required_W'],
...
def generate_report(optimization_results):
    ...
    report = {
        ...
        'n_feasible': len(optimization_results['all_feasible']),
        'optimal_designs': {},
    }
    ...
    for tier, design in optimization_results['best_by_tier'].items():
        ...
        report['optimal_designs'][tier] = {'design': design, 'sensitivity': sens}
