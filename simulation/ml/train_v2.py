"""Train ArgusFusionNet (architecture per architecture.svg).

    python train_v2.py --epochs 15 --out argus_v2.pt
"""
...
from model_v2 import ArgusFusionNet
...
def main():
    ...
    ap.add_argument("--out", type=str, default="argus_v2.pt")
    ...
    model = ArgusFusionNet(seq_len=seq_len, horizon=horizon).to(device)
    ...
    cls_w, rs_w, pos_w, vel_w = 0.5, 0.4, 1.0, 1.0
    ...
    for epoch in range(args.epochs):
        ...
        for xb, cb, rb, tb in train_dl:
            ...
            if mask.any():
                ...
                pred_traj = out["traj"][mask].contiguous()
                tgt_traj = tb[mask].contiguous()
                pos_loss = F.smooth_l1_loss(pred_traj[..., :3].contiguous(), tgt_traj[..., :3].contiguous())
                vel_loss = F.smooth_l1_loss(pred_traj[..., 3:].contiguous(), tgt_traj[..., 3:].contiguous())
            else:
        ...
        model.eval()
        v_cls_loss = 0.0; v_acc = 0.0
        v_pos_mae = 0.0; v_vel_mae = 0.0; v_dir_cos = 0.0
        n = 0; pos_n = 0
        with torch.no_grad():
            for xb, cb, rb, tb in val_dl:
                ...
                mask = cb > 0.5
                if mask.any():
                    p_err = (out["traj"][mask].contiguous()[..., :3] - tb[mask][..., :3]).abs().mean() * pos_norm
                    v_err = (out["traj"][mask].contiguous()[..., 3:] - tb[mask][..., 3:]).abs().mean() * 2500.0
                    p_v = out["traj"][mask].contiguous()[:, 0, 3:]
                    t_v = tb[mask][:, 0, 3:]
